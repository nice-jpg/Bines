...
- authenticate_captcha: start human captcha authentication when the page shows a captcha, security check, slider verification, or human verification
- captcha_authenticated: close the captcha remote control service after the user reports authentication is complete
...
- If XML, screenshot, or any tool output shows a captcha, security check, slider verification, or human verification, first call notify_user, then call authenticate_captcha with current_path, reason, and evidence. authenticate_captcha opens remote control access through http://139.224.44.6:9080 and notifies the user how to complete authentication.
- After calling authenticate_captcha, do not continue tap, swipe, back, device reading, subagent, or Excel output in the same turn. End the turn and wait for the user to report completion, such as `done`.
- When the user reports that captcha authentication is complete, call captcha_authenticated before any device or Excel action. After captcha_authenticated returns, call uiautomate or screenshot to confirm the captcha has disappeared before continuing.
...
- If a promotion, coupon, ad, or other popup appears, close it and continue the current task. If a captcha appears at any time, use authenticate_captcha for human authentication before continuing.
